...
export function activate(context: vscode.ExtensionContext) {
  console.log(
    'Congratulations, your extension "image-color-palette" is now active!'
  );

  // hello world 示例命令
  const disposable = vscode.commands.registerCommand(
    "image-color-palette.helloWorld",
    () => {
      vscode.window.showInformationMessage(
        "Hello World from image-color-palette!"
      );
    }
  );
...
async function ensureTargetImage(
  ...
  if (!IMAGE_EXTS.includes(ext)) {
    vscode.window.showWarningMessage(
      "仅支持图片文件：png/jpg/jpeg/gif/webp/bmp/svg"
    );
    return undefined;
  }
  return uri;
...
function openPaletteWebview(
  ...
  const scriptUri = panel.webview.asWebviewUri(
    vscode.Uri.file(path.join(context.extensionPath, "media", "paletteView.js"))
  );
  // const styleUri = panel.webview.asWebviewUri(
  //   vscode.Uri.joinPath(context.extensionUri, "media", "palette.css")
  // );
  const styleUri = panel.webview.asWebviewUri(
    vscode.Uri.file(path.join(context.extensionPath, "media", "palette.css"))
  );
